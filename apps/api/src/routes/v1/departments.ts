...
export async function departmentRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/", { preHandler: requirePermission("ADM_DEPARTMENTS", "canCreate") }, async (request, reply) => {
  ...
  fastify.patch("/:id", { preHandler: requirePermission("ADM_DEPARTMENTS", "canEdit") }, async (request, reply) => {
  ...
  fastify.delete("/:id", { preHandler: requirePermission("ADM_DEPARTMENTS", "canDelete") }, async (request, reply) => {
    const { id } = request.params as { id: string };
